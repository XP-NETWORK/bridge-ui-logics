...
const polkadotPalletHelperFactory = async (node_uri) => {
  ...
  return Object.assign(Object.assign({}, base), {
    ...
    async balanceWrappedBatch(address, chain_nonces) {
      ...
      const res = await api.query.erc1155.balances.multi(
      ...
      return new Map(
        res.map((b, i) => [
          chain_nonces[i],
          b.isSome
            ? new bignumber_js_1.default(b.unwrap().toString())
    ...
    decodeWrappedNft(raw_data) {
      const packed = encoding_1.NftPacked.deserializeBinary(
        Uint8Array.from(raw_data)
      )
      return {
    ...
    async decodeUrlFromRaw(data) {
      const locked = await getLockedNft(
        new types_1.U8aFixed(api.registry, data, 256)
      )
      if (locked === undefined) {
