...
async function baseWeb3HelperFactory(provider) {
  ...
  return {
    ...
    async deployErc1155(owner) {
      const factory = ethers_1.ContractFactory.fromSolidity(
        ERC1155_contract,
        owner
      )
    ...
    async mintNft(contract_owner, { contract, token, owner, uri }) {
      const erc1155 = new ethers_1.Contract(
        contract,
        erc1155_abi,
        contract_owner
      )
 ...
 */
async function web3HelperFactory(
  provider,
  minter_addr,
  minter_abi,
  erc1155_addr
  ...
  async function nftUri(info) {
    if (info.contract_type == 'ERC721') {
      const erc = new ethers_1.Contract(
        info.contract,
        fakeERC721_json_1.abi,
        w3
      )
  ...
  return Object.assign(Object.assign({}, base), {
    ...
    async balanceWrappedBatch(address, chain_nonces) {
      const bals = await erc1155.balanceOfBatch(
      ...
      return new Map(
        bals.map((v, i) => [
          chain_nonces[i],
          new bignumber_js_1.default(v.toString()),
        ])
      )
    },
